...
POLL_TYPE_BASIC, \
    ...
    POLL_TYPE_DOODLE = list(range(12))
...
# Conversation states:
NOT_ENGAGED, TYPING_TITLE, TYPING_TYPE, TYPING_OPTION, TYPING_META = list(range(5))
...
class PollBot:
    ...
    def poll(self, update, context):
        """Send a message when the command /poll is issued."""
        update.message.reply_text('Please send me title for poll. (/cancel to exit)')

        return TYPING_TITLE

    def handle_type(self, update, context):
        text = update.message.text
        polltype = next((i for i, handler in list(POLL_HANDLERS.items()) if handler.name == text), None)
        ...
        if POLL_HANDLERS[polltype].requires_extra_config(context.user_data['meta']):
            ...
            update.message.reply_text("{}. Now, send me the emoji (or /cancel)".format(self.get_affirmation()))
    ...
    def assemble_reply_keyboard(self):
        keyboard = []
        for _, val in list(POLL_HANDLERS.items()):
    ...
    def assemble_type_regex(self):
        orclause = '|'.join([handler.name for handler in list(POLL_HANDLERS.values())])
    ...
    def send_help(self, update, context):
        ...
        for poll in list(POLL_HANDLERS.values()):
            helptext += "*{}*: _{}_\n".format(
                poll.name,
                poll.desc
            )
